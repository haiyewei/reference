...
*`test` [属性](../attributes.md)* 将函数标记为要作为测试执行。
...
`test` 属性只能应用于单态、不接受参数且返回类型实现了 [`Termination`](std::process::Termination) trait 的[自由函数](../glossary.md#free-item)。

> [!NOTE]
> 实现 [`Termination`](std::process::Termination) trait 的一些类型包括：
> * `()`
...
`test` 属性由标准库 prelude 以 [`std::prelude::v1::test`] 导出。
...
测试框架会调用返回值的 [`report`](std::process::Termination::report) 方法，并根据产生的 [`ExitCode`](std::process::ExitCode) 是否表示成功终止，将测试归类为通过或失败。具体来说：
...
>     let state = setup_the_thing()?; // 预期会成功
>     do_the_thing(&state)?;          // 预期会成功
>     Ok(())
> }
> ```
...
*`ignore` [属性](../attributes.md)* 可以与 [`test` 属性][attributes.testing.test] 一起使用，用来告知测试框架不要将该函数作为测试执行。
...
*`should_panic` [属性](../attributes.md)* 使测试仅在应用该属性的[测试函数][attributes.testing.test] panic 时通过。
...
- [MetaNameValueStr] --- 给定字符串必须出现在 panic 消息中，测试才会通过。
  > [!EXAMPLE]
  > ```rust,no_run
  > #[test]
...
- [MetaListNameValueStr] --- 与 [MetaNameValueStr] 语法一样，给定字符串必须出现在 panic 消息中。
  > [!EXAMPLE]
  > ```rust,no_run
  > #[test]
...
使用 [MetaNameValueStr] 形式，或使用带有 `expected` 键的 [MetaListNameValueStr] 形式时，给定字符串必须出现在 panic 消息中的某处，测试才会通过。
