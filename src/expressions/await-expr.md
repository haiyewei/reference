...
2. 将该 future 求值为一个 [future](../../core/future/future/trait.Future.html) `tmp`；
3. 使用 [`Pin::new_unchecked`](../../core/pin/struct.Pin.html#method.new_unchecked) 固定 `tmp`；
4. 随后通过调用 [`Future::poll`](../../core/future/future/trait.Future.html#tymethod.poll) 方法并向其传入当前[任务上下文](#task-context)来轮询这个已固定的 future；
5. 如果对 `poll` 的调用返回 [`Poll::Pending`](../../core/task/poll/enum.Poll.html#variant.Pending)，则该 future 返回 `Poll::Pending`，并挂起其状态，以便在外围 async 上下文再次被轮询时，执行返回到步骤 3；
6. 否则，对 `poll` 的调用必定已返回 [`Poll::Ready`](../../core/task/poll/enum.Poll.html#variant.Ready)，在这种情况下，[`Poll::Ready`](../../core/task/poll/enum.Poll.html#variant.Ready) 变体中包含的值会用作 `await` 表达式本身的结果。
