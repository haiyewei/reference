...
r[statement.intro]
*语句*是[块](expressions/block-expr.md)的组成部分，而块又是外层[表达式](expressions.md)或[函数](items/functions.md)的组成部分。
...
*声明语句*会向外围语句块中引入一个或多个*名称*。被声明的名称可以表示新的变量或新的[项](items.md)。
...
*项声明语句*的语法形式与[模块](items/modules.md)内的[项声明](items.md)相同。
...
*`let` 语句*会引入一组新的[变量](variables.md)，这些变量由[模式](patterns.md)给出。模式后面可以跟随类型标注，然后要么到此结束，要么跟随初始化表达式以及可选的 `else` 块。
...
let Some(t) = v.pop() else { // 可反驳模式需要 else 块
    panic!(); // else 块必须发散
};
let [u, v] = [v[0], v[1]] else { // 该模式是不可反驳的，因此编译器
...
*表达式语句*会对一个[表达式](expressions.md)求值并忽略其结果。通常，表达式语句的目的是触发对其表达式求值所产生的效果。
...
[ExpressionWithBlock] 表达式在用作语句时，其类型必须是单元类型。
